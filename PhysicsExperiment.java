 ...
 */
//package physicsexperiment;

/**
 * Physics Experiment
 * Author: Asma Pahsa and Carolyn Yao
...
public class PhysicsExperiment {
	...
	public int[][] scheduleExperiments(int numStudents, int numSteps, int[][] signUpTable) {
		...
		int presentStudent = 1; // take count of pointing student
		int presentStep = 1; // take count of pointing step
		int check = numSteps;
		while (check > 0) {
			...
			if (signUpTable[presentStudent][presentStep] == 0) {
				if (presentStep != numSteps) {
					...
					for (int row = 1; row <= numStudents; row++) {
						...
						for (int col = presentStep; col <= numSteps; col++) {
							...
							// updating data depend on the student link
							if (joinedLink > link) {
								link = joinedLink;
								point = row;
							}
					...
					presentStudent = point; // updating present student count
				}

				else {
					// checking the present student if last one then select the first one
					if (presentStudent == numStudents)
						presentStudent = 1;
				...
				check = check - 1; // decrease the remaining number of check
			}
		}
